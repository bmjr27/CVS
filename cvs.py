...
response = requests.get(
    'https://www.cvs.com/shop-assets/proxy/search?pnp=true&normalizer=%22productIndex%22&normalizerArgs=%7B%7D&query=%22%22&skip=0&pageSize=20&orFields=%5B%22variants.subVariant.availability%22%5D&refinements=%5B%5D',
...
for product in data['products']:
    ...
    # Extract product URL
    product_url = product['variants'][0]['url']  # URL of the product
    
    # Extract the URL of the first image
    first_image_url = product['images'][0]['uri']
    
    # Extract coupons and shipping eligibility for each variant
    for variant in product['variants']:
        variant_coupons = variant.get('cvsCoupons', [])
        variant_shipping_eligibility = {
            "pickupEligible": variant['pickupEligible'],
            "twoDayShipping": variant['twoDayShipping'],
            "sameDayEligible": variant['sameDayEligible']
        }
        ...
        print("Variant URL:", 'https://www.cvs.com' + variant['url'])
        print("Variant Coupons:")
        for coupon in variant_coupons:
            print("Coupon Number:", coupon['cpnNbr'], "   End Date:", coupon['end_dt'])
      
        print("Variant Shipping Eligibility:", variant_shipping_eligibility)
